<template>
        ...
        <v-col cols="12" md="12">
            <div class="row d-block justify-content-center">
                <v-btn class="btn-block" @click="savecustomer()" dark color="teal darker-3">
                    <v-icon>mdi-content-save</v-icon> Save</v-btn
                >
                <v-btn class="btn-block"  @click="cancelcustomer()" dark color="red darker-3"
...
export default {
    ...
    methods: {
        ...
        //saving customer
        savecustomer(){
            this.actionNow = 'save_customer';
            this.dialogTitle = "Saving customer!"
            this.dialogBody = "Are you sure about saving this customer!"
            this.dialog = true;

        },
        cancelcustomer(){
            this.actionNow = 'cancel_customer';

            this.dialogTitle = "Cancel Saving customer!"
            this.dialogBody = "Are you sure about cancelling, saving this customer!"
            this.dialog = true;
        },
        //end saving customer
        ...
        agree(){
            this.dialog = false;
            if(this.actionNow === 'save_customer') this.submitcustomer();
            else if(this.actionNow === 'cancel_customer') this.submitcustomer();
        },
        disagree(){
            this.dialog = false;
            if(this.actionNow === 'save_customer') this.submitcustomer();
        ...
        submitcustomer(){
            let data = {};
            if(this.actionNow === 'save_customer'){
                ...
                this.$store.dispatch('customersModule/SAVE_CUSTOMER_ACTION',data).then(res=>{

                    const {errors,message} = res;

                    if(message !== undefined){
                        this.$toast.success({
                        title: "Saved Customer Well!",
                        message: message,
                        color: "#388E3C",
                        timeOut: 5000,
                        showMethod: "lightSpeedIn",
            ...
            else if(this.actionNow === 'cancel_customer'){
